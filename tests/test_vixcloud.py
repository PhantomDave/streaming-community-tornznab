from app.vixcloud import extract_m3u8_url, parse_master_playlist, settings
...
def test_extract_m3u8_url() -> None:
    ...
    assert extract_m3u8_url(html) == "https://cdn.example/video/master.m3u8?token=123"
...
v2160.m3u8
"""
    variants = parse_master_playlist(playlist, "https://cdn.example/master.m3u8")
...
def test_extract_m3u8_url_returns_none_when_missing() -> None:
    assert extract_m3u8_url("<html></html>") is None
...
def test_parse_master_playlist_resolves_separate_audio_rendition() -> None:
    ...
    variants = parse_master_playlist(_PLAYLIST_WITH_SEPARATE_AUDIO, "https://vixcloud.co/master.m3u8")
